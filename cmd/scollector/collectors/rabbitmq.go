...
func rabbitmqQueues(s string) (opentsdb.MultiDataPoint, error) {
	...
	var qs []rmqQueue
	if err := json.NewDecoder(bytes.NewReader(content[:])).Decode(&qs); err != nil {
		return nil, err
	}
	var md opentsdb.MultiDataPoint
	for _, q := range qs {
		host := strings.Split(q.Node, "@")[1]
...
func rabbitmqNodes(s string) (opentsdb.MultiDataPoint, error) {
	...
	var ns []rmqNode
	if err := json.NewDecoder(bytes.NewReader(content[:])).Decode(&ns); err != nil {
		return nil, err
	}
	var md opentsdb.MultiDataPoint
	for _, n := range ns {
		host := strings.Split(n.Name, "@")[1]
		ts := opentsdb.TagSet{"host": host}
...
func rabbitmqBackingQueueStatus(p string, ts opentsdb.TagSet, bqs rmqBackingQueueStatus) opentsdb.MultiDataPoint {
	...
	Add(&md, p+"avg_rate", bqs.AvgAckEgressRate, ts.Copy().Merge(opentsdb.TagSet{"method": "ack", "direction": "in"}),
